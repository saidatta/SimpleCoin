...
import threading
import time
import simplecoin.util as sutil
...
from simplecoin.hash import sha256
from simplecoin.crypto import get_new_priv_key, get_pub_key, get_address
from simplecoin.structure import Vin, Vout, Coinbase, Transaction, Block
from simplecoin.synchronization import simplecoin_cli, simplecoinRequestHandler

# Private Key to the genesis_block's output address is
# sha256('simple is a good boy')
# which is
...
def add_to_blockchain(block: Block):
    ...
    if (block.height - 1) not in global_blockchain:
        for node in global_nodes:
            try:
                missing_block_dict: Dict = simplecoin_cli(
    ...
    if block.height not in global_blockchain:
        ...
        for tx in block.transactions:
            if tx.txid not in global_txs:
                # Update utxos
                global_best_block, global_txs, global_utxos = sutil.add_tx_to_block(
        ...
        for node in global_nodes:
            try:
                simplecoin_cli('receive_mined_block', [
...
@dispatcher.add_method
def send_simplecoin(to_address: str, amount: int):
    global global_utxos
    ...
    try:
        ...
        if (accumulated_amount < send_amount):
            return {'error': 'You\'re trying to send {} simplecoin when you have {} simplecoin'.format(send_amount, accumulated_amount)}
        ...
        for i in range(len(tx.vins)):
            tx = sutil.sign_tx(tx, i, account_priv_key)
...
def sign_raw_tx(tx: str, idx: int, pk: str):
    try:
        ...
        signed_tx = sutil.sign_tx(tx, idx, pk)
...
def sync_with_nodes():
    ...
    for node in global_nodes:
        try:
            simplecoin_cli('init_connection', [global_host, global_port], **node)
        except:
            pass
    ...
    while True:
        ...
        for node in global_nodes:
            ...
            try:
                node_best_length = simplecoin_cli(
        ...
        if longest_node is not None:
            latest_block_dict: Dict = simplecoin_cli(
...
def run_simplecoin(host='localhost', port=4000, nodes=['localhost:4000'], **kwargs):
    t1 = threading.Thread(target=partial(
        run_simple, threaded=True, request_handler=simplecoinRequestHandler), args=(host, port, simplecoin_app))
    t1.daemon = True
    t1.start()
...
if __name__ == '__main__':
    ...
    print('** [Welcome] Your simplecoin address is {}'.format(account_address))

    run_simplecoin(**config_kwargs)